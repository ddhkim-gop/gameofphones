#!/usr/bin/python3
"""
Local Sleeper poller for Game of Phones.  [obi]
...
2026-08-04). `workflow_dispatch` is NOT throttled, so we poll Sleeper from this
Mac and dispatch the workflow only when something actually changed.
...
LEAGUE_ID = "1313903635586899968"          # 2026 — keep in step with refresh_transactions.py
REPO = "ddhkim-gop/gameofphones"
WORKFLOW = "auto_refresh.yml"
GH = "/opt/homebrew/bin/gh"

STATE_DIR = os.path.expanduser("~/Library/Application Support/gameofphones")
...
def get(url):
    req = urllib.request.Request(url, headers={"User-Agent": "gameofphones-poll/1.0"})
