...
// ── Value system ──────────────────────────────────────────────────────────────
// Convert positional rank → 0–100 score. Curve: rank 1 = 100, rank 12 ≈ 55, rank 24 ≈ 23, rank 36 ≈ 0
function rankToScore(rank) {
    if (!rank || rank < 1) return 0;
    return Math.max(0, Math.round(100 - Math.pow(rank - 1, 0.72) * 8));
}

function playerIdFor(name) { return nameToId[name] || null; }

// Get value score for a player in a given year (0 if no data)
function playerYearScore(name, year) {
    const pid = playerIdFor(name);
    if (!pid) return null;
    const st = (statsCache[year] || {})[pid];
    if (!st || !st.rank) return null;
    return { score: rankToScore(st.rank), rank: st.rank, position: st.position };
}
...
// Average value score across post-trade years for a player
function playerPostTradeValue(name, tradeYear) {
    const years = postTradeYears(tradeYear);
    if (!years.length) return { avg: null, byYear: [] };
    const byYear = years.map(y => ({ year: y, data: playerYearScore(name, y) }));
    const valid = byYear.filter(e => e.data !== null);
    const avg = valid.length ? valid.reduce((s, e) => s + e.data.score, 0) / valid.length : null;
    return { avg, byYear };
}

// Pick value: resolved player's post-trade value, or round-based estimate
function pickPostTradeValue(asset, receivingTeam, tradeYear) {
    ...
    if (resolved) {
        ...
        const byYear = ptYears.map(y => ({ year: y, data: playerYearScore(resolved.player, y) }));
        const valid  = byYear.filter(e => e.data !== null);
        const avg    = valid.length ? valid.reduce((s, e) => s + e.data.score, 0) / valid.length : null;
        return { avg, byYear, resolved: resolved.player, resolvedPos: resolved.position, estimated: false };
    }

    // Unresolved — estimate by round
    const baseVal = parseInt(pickYear) > 2025 ? null : // future pick, no data
        round === 1 ? 50 : round === 2 ? 28 : 12;
    return { avg: baseVal, byYear: [], resolved: null, estimated: baseVal !== null };
}

// Compute full side value
...
function tradeNarrative(tx, itemsA, itemsB, valA, valB) {
    ...
    if (noData) {
    ...
    } else if (Math.abs(valA - valB) < 5) {
        parts.push(`This trade was roughly even. Both sides generated comparable post-trade value (${valA.toFixed(0)} vs ${valB.toFixed(0)}).`);
    } else {
        ...
        parts.push(`<strong>${winner}</strong> wins this trade by ${descriptor} — ${winVal} post-trade value vs ${loseVal} for ${loser}.`);
    }

    // Standout performers
    if (!noData) {
        const allItems = [
            ...itemsA.filter(i => !i.isPick && i.avg !== null).map(i => ({...i, team: teamA})),
            ...itemsB.filter(i => !i.isPick && i.avg !== null).map(i => ({...i, team: teamB})),
        ].sort((a,b) => (b.avg ?? 0) - (a.avg ?? 0));

        const elite = allItems.filter(i => (i.avg ?? 0) >= 60);
        const busts = allItems.filter(i => (i.avg ?? 0) < 15);

        if (elite.length > 0) {
            const e = elite[0];
            const yearSummary = e.byYear.filter(y => y.data).map(y => `${y.year}: #${y.data.rank} ${y.data.position}`).join(", ");
            parts.push(`The best asset in the deal was <strong>${e.name}</strong> (${teamA === e.team ? teamA : teamB}), who delivered elite production after the trade${yearSummary ? ` — ${yearSummary}` : ""}.`);
        }
        if (elite.length > 1) {
            const e2 = elite[1];
            parts.push(`<strong>${e2.name}</strong> was also a quality return, averaging a value score of ${e2.avg.toFixed(0)}/100 in the post-trade window.`);
        }
        if (busts.length > 0 && busts[0].avg === 0) {
            parts.push(`<strong>${busts[0].name}</strong> contributed almost nothing after the trade — a clear bust on the deal.`);
        }
    }
    ...
    if (postYears.length === 1) {
        parts.push(`Note: only ${postYears[0]} data is available; value scores will update when more seasons complete.`);
    }
...
function scoreBar(score, max = 100) {
    ...
    const color = score >= 70 ? "#3ecf8e" : score >= 40 ? "#f6ad55" : "#f87171";
...
function renderItemCard(item, tradeYear) {
    ...
    if (item.isPick) {
        ...
        if (item.resolved) {
            ...
            <div style="margin-top:5px;padding:5px 8px;background:#1a1c22;border-radius:6px;border-left:2px solid #3d4350;">
                ...
                ${item.avg !== null ? `<div style="font-size:10px;color:#8b9099;">Post-trade value: <strong style="color:#f0f1f3;">${item.avg.toFixed(0)}/100</strong></div>${scoreBar(item.avg)}` : ""}
            </div>`;
        } else if (item.estimated && !future) {
            resolvedHtml = `<div style="font-size:10px;color:#5a6070;margin-top:3px;">Est. value: ~${item.avg}/100 (pick unresolved)</div>`;
        } else {
            resolvedHtml = `<div style="font-size:10px;color:#5a6070;margin-top:3px;">${future ? "Future pick" : "Pick data unavailable"}</div>`;
        }
    ...
    const avgColor = item.avg === null ? "#5a6070" : item.avg >= 70 ? "#3ecf8e" : item.avg >= 40 ? "#f6ad55" : "#f87171";
    const yearRows = item.byYear.map(e => {
        if (!e.data) return `<span style="color:#5a6070;">${e.year}: —</span>`;
        return `<span>${e.year}: <strong style="color:#c9cdd4;">#${e.data.rank} ${e.data.position}</strong> <span style="color:#5a6070;">(${rankToScore(e.data.rank)}/100)</span></span>`;
    }).join(" · ");

    return `
    <div style="margin-bottom:10px;">
        <div style="display:flex;align-items:center;gap:5px;margin-bottom:3px;">
            ${posTag(item.position)}
...
function renderTradeCard(tx) {
    ...
    const col = (team, items, isWinner) => {
        const isLoser = winner !== null && !isWinner && winner !== null;
        return `
        <div style="flex:1;min-width:0;padding:0 14px 0 0;">
            <div style="display:flex;align-items:center;gap:7px;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid #2d3139;">
                <span style="font-size:13px;font-weight:800;color:${isWinner ? "#3ecf8e" : isLoser ? "#8b9099" : "#f0f1f3"};">${team}</span>
                ${isWinner ? '<span style="font-size:10px;font-weight:700;color:#3ecf8e;background:#0d2b1e;border-radius:4px;padding:2px 6px;">WIN</span>' : ""}
            ...
            ${hasData ? `<div style="font-size:10px;color:#5a6070;border-top:1px solid #2d3139;padding-top:5px;margin-top:4px;">Total post-trade value: <strong style="color:#c9cdd4;">${(isWinner ? Math.max(valA,valB) : Math.min(valA,valB)).toFixed(0)}</strong></div>` : ""}
        </div>`;
    };
    ...
    <div style="background:#1e2027;border:1px solid #2d3139;border-radius:12px;padding:16px 18px;margin-bottom:18px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1px 1fr;gap:0;margin-bottom:14px;">
            ${col(teamA, itemsA, winner === teamA)}
            <div style="background:#2d3139;"></div>
            <div style="padding-left:14px;">${col(teamB, itemsB, winner === teamB).replace('padding:0 14px 0 0', 'padding:0')}</div>
        </div>
...
function renderAll(year, showPreseason) {
    ...
    const notice = ptYears.length === 0
        ...
        ? `<div style="background:#1a2230;border:1px solid #2d4060;border-radius:8px;padding:10px 14px;font-size:12px;color:#8b9099;margin-bottom:20px;">
            Value based on ${ptYears[0]} only — scores will refine as more seasons complete.
           </div>` : "";
...
async function init() {
    ...
    <div style="font-size:12px;color:#5a6070;margin-bottom:20px;">
        Post-trade value (0–100) is based on positional rank in seasons after the trade. Win/loss reflects which side accumulated more post-trade value.
    </div>
    ...
    // Load stats for all real years + draft for all years
    await Promise.all([
