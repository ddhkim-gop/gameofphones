...
function render() {
    ...
    if (userVal !== "all") {
        filtered = filtered.filter(t => {
            if (t.type === "trade") return (t.teams || []).includes(userVal);
            return t.team === userVal;
        });
    }
...
async function init() {
    ...
    try {
        ...
        const sortedUsers = (usersList || []).map(u => u.username).sort();
        document.getElementById("filterUser").innerHTML =
            `<option value="all">All Users</option>` +
            sortedUsers.map(u => `<option value="${u}">${u}</option>`).join("");
